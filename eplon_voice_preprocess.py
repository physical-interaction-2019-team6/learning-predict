...
"""
    waveファイルを読み込む
"""
def _eplon_voice_read_wave(file_name):
    try:
...
"""
#max mean
def _eplon_voice_discrimination_algorithm_max_mean(whole_mean, part_max):
    return (part_max  > whole_mean)

#mean mean
def _eplon_voice_discrimination_algorithm_mean_mean(whole_mean, part_mean):
    return (part_mean  > whole_mean)

def _eplon_voice_discrimination_algorithm_therhold(part_mean):
...
"""
def eplon_voice_read(file_name):
    """読み込み作業"""
    wave_file = _eplon_voice_read_wave(file_name)
...
"""
def eplon_voice_crop(buffers, sample_rate):
    ...
    for i in range(iteral_max):
        ...
        #part_max    = np.max(np.abs(cropped))       #区間の最大値
        part_mean   = np.mean(np.abs(cropped))       #区間の最大値

        #区間に音声が含まれているか識別
        if(not _eplon_voice_discrimination_algorithm_therhold(part_mean)):
            continue
        cropped_buffers.append(cropped)

    print("ABAIRABLE AREA is : "+str(len(cropped_buffers))+"/"+str(iteral_max)+" ("+str(int(len(cropped_buffers)*100/iteral_max))+"%)")
    print("CROP BUFFERS SIZE : ("+str(len(cropped_buffers))+", "+str(cropped_buffers[0].shape))
...
"""
    ...
    return ペリオドグラム
"""
def eplon_voice_periodogram(data):
